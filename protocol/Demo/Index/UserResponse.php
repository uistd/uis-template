...
class UserResponse extends Result
{
    ...
    {
        $result = array();
        if (isset($this->data) && $this->data instanceof UserData) {
    ...
    public function binaryEncode($pid = false, $sign = false, $mask_key = null)
    {
        ...
        if ($pid) {
            $result->writePid('/demo/indexUserResponse');
        }
        if ($sign) {
            $result->sign();
        }
    ...
    public static function binaryStruct()
    {
        ...
        $byte_array->writeString(UserData::binaryStruct());
