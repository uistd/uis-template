...
class MainResponse extends Result
{
    ...
    {
        $result = array();
        if (isset($this->data) && $this->data instanceof MainData) {
    ...
    public function binaryEncode($pid = false, $sign = false, $mask_key = null)
    {
        ...
        if ($pid) {
            $result->writePid('/demo/indexMainResponse');
        }
        if ($sign) {
            $result->sign();
        }
        ...
        if (!$this->data instanceof MainData ) {
            $result->writeChar(0);
        } else {
    ...
    public static function binaryStruct()
    {
        ...
        $byte_array->writeString(MainData::binaryStruct());
